...
if [[ "$INPUT" == search:* ]]; then
  ...
  URL=$(yt-dlp "ytsearch:$QUERY" --get-url --no-playlist -f bestaudio 2>/dev/null | head -1)
  if [ -z "$URL" ]; then
  ...
  if [ -z "$NAME" ]; then
    NAME=$(yt-dlp "ytsearch:$QUERY" --get-title --no-playlist 2>/dev/null | head -1 | \
